...
from jax.experimental.stax import serial, Dense, elu, Sigmoid, glorot_normal, normal
...
class FlexTENet(BaseCATENet):
    ...
    """
    def __init__(self, binary_y: bool = False,
...
def train_flextenet(X, y, w, binary_y: bool = False,
                    n_layers_out: int = DEFAULT_LAYERS_OUT,
                    n_units_s_out: int = DEFAULT_DIM_S_OUT,
                    n_units_p_out: int = DEFAULT_DIM_P_OUT,
                    n_layers_r: int = DEFAULT_LAYERS_R,
                    ...
                    n_units_r=None,
                    n_units_out=None):  # TODO incorporate different nonlins here
    ...
    # get output head
    init_fun, predict_fun = FlexTENetArchitecture(n_layers_out=n_layers_out,
    ...
    if not binary_y:
        ...
        @jit
        def loss(params, batch, penalty_l2, penalty_l2_p, penalty_orthogonal, mode):
            # mse loss function
            inputs, targets = batch
            preds = predict_fun(params, inputs, mode=mode)
            penalty = _compute_penalty(params, n_layers_out, n_layers_r, private_out,
                                       penalty_l2, penalty_l2_p, penalty_orthogonal,
                                       shared_repr, normalize_ortho, mode)
    ...
    # set update function
    @jit
    def update(i, state, batch, penalty_l2, penalty_l2_p, penalty_orthogonal, mode):
    ...
    if not pretrain_shared:  # train entire model together
        for i in range(n_iter):
            ...
            for b in range(n_batches):
                ...
                opt_state = update(i * n_batches + b, opt_state, next_batch, penalty_l2,
                                   penalty_l2_p, penalty_orthogonal, mode=1)

            if (verbose > 0 and i % n_iter_print == 0) or early_stopping:
                params_curr = get_params(opt_state)
                l_curr = loss(params_curr, ((X_val, w_val), y_val), penalty_l2, penalty_l2_p,
                              penalty_orthogonal, mode=1)
        ...
        for i in range(n_iter):
            ...
            if verbose > 0 and i % n_iter_print == 0:
                print("Pre-training epoch: {}, current {} loss: {}".format(i, val_string, l_curr))
        ...
        # set update function
        @jit
        def update2(i, state, batch, penalty_l2, penalty_l2_p, penalty_orthogonal, mode):
            params = get_params(state)
            g_params = grad(loss)(params, batch, penalty_l2, penalty_l2_p, penalty_orthogonal,
                                  mode)
        ...
        for i in range(n_iter):
            ...
            for b in range(n_batches):
                idx_next = train_indices[(b * batch_size):min((b + 1) * batch_size, n - 1)]
                next_batch = (X[idx_next, :], w[idx_next]), y[idx_next, :]
                opt_state = update2(i * n_batches + b, opt_state, next_batch, penalty_l2,
                                    penalty_l2_p, penalty_orthogonal, mode=1)
...
def predict_flextenet(X, trained_params, predict_funs, return_po: bool = False,
...
def _get_cos_reg(params_0, params_1, normalize):
    if normalize:
...
def _compute_ortho_penalty_asymmetric(params, n_layers_out, n_layers_r,
                                      private_out, penalty_orthogonal,
                                      shared_repr,
                                      normalize_ortho, mode=1):
    ...
    n_in = [params[i][0][0].shape[0] for i in
            range(lb, 2 * (n_layers_out + n_layers_r), 2)]

    ortho_body = _get_cos_reg(params[lb][1][0], params[lb][2][0], normalize_ortho)
    ortho_body = ortho_body + \
                     sum([_get_cos_reg(params[i][0][0],
                                       params[i][1][0][:n_in[int(i / 2 - lb / 2)], :],
                                       normalize_ortho) + _get_cos_reg(params[i][0][0],
                                                                       params[i][2][0][
                                                                       :n_in[int(i / 2 - lb / 2)],
                                                                       :], normalize_ortho)
    ...
    if not private_out:
        ...
        n_in = params[idx_out][0][0].shape[0]

        ortho_body = ortho_body + _get_cos_reg(params[idx_out][0][0], params[idx_out][1][0][
                                                                          :n_in, :],
                                                   normalize_ortho) + \
                         _get_cos_reg(params[idx_out][0][0], params[idx_out][2][0][:n_in, :],
                                      normalize_ortho)

    return mode * penalty_orthogonal * ortho_body


def _compute_penalty_l2(params, n_layers_out, n_layers_r, private_out, penalty_l2,
                        penalty_l2_p, shared_repr, mode=1):
    n_bodys = N_SUBSPACES

    # compute l2 penalty
    if shared_repr:
        # get representation and then heads
        weightsq_body = penalty_l2 * sum([jnp.sum(params[i][0] ** 2) for i in
                                          range(0, 2 * n_layers_r, 2)])
        weightsq_body = weightsq_body + penalty_l2 * sum([jnp.sum(params[i][0][0] ** 2) for i in
                                                          range(2 * n_layers_r,
                                                                2 * (n_layers_out + n_layers_r),
                                                                2)])
        weightsq_body = weightsq_body + penalty_l2_p * \
                        mode * sum([sum([jnp.sum(params[i][j][0] ** 2)
                                         for i in range(2 * n_layers_r,
                                                        2 * (n_layers_out + n_layers_r), 2)])
                                    for j
                                    in range(1, n_bodys)])
    else:
        weightsq_body = penalty_l2 * sum([jnp.sum(params[i][0][0] ** 2) for i in
                                          range(0, 2 * (n_layers_out + n_layers_r), 2)])
        weightsq_body = weightsq_body + penalty_l2_p * mode * sum([sum([jnp.sum(params[i][j][0] **
                                                                                2) for i in
                                                                        range(0, 2 * (
                                                                                    n_layers_out +
                                                                                    n_layers_r),
    ...
    idx_out = 2 * (n_layers_r + n_layers_out)
    if private_out:
        weightsq = weightsq_body + penalty_l2 * jnp.sum(params[idx_out][0][0] ** 2) + \
...
def _compute_penalty(params, n_layers_out, n_layers_r, private_out, penalty_l2, penalty_l2_p,
                     penalty_orthogonal, shared_repr, normalize_ortho,
                     mode=1):
    l2_penalty = _compute_penalty_l2(params, n_layers_out, n_layers_r, private_out, penalty_l2,
                                     penalty_l2_p, shared_repr, mode)

    ortho_penalty = _compute_ortho_penalty_asymmetric(params, n_layers_out, n_layers_r,
                                                      private_out, penalty_orthogonal,
                                                      shared_repr, normalize_ortho, mode)
...
def SplitLayerAsymmetric(n_units_s, n_units_p, first_layer: bool = False, same_init: bool = True):
    ...
    def init_fun(rng, input_shape):
        ...
        out_shape = input_shape[0][:-1] + (n_units_s,), input_shape[1][:-1] + (n_units_p + n_units_s,), \
        ...
        if same_init:  # use same init for the two private layers
            return out_shape, (init_s(rng_1, input_shape[0])[1], init_p(rng_2, input_shape[1])[1],
...
def TEOutputLayerAsymmetric(private=True, same_init: bool = True):
    ...
    if private:
        ...
        def init_fun(rng, input_shape):
            ...
            return out_shape, (init_f(rng_1, input_shape[1])[1], init_f(rng_2, input_shape[2])[1])
        ...
        def init_fun(rng, input_shape):
            ...
            if same_init:
                return out_shape, (
                    init_f(rng_1, input_shape[0])[1], init_f(rng_2, input_shape[1])[1],
...
def FlexTENetArchitecture(n_layers_out: int = DEFAULT_LAYERS_OUT,
                          n_units_s_out: int = DEFAULT_DIM_S_OUT,
                          n_units_p_out: int = DEFAULT_DIM_P_OUT,
                          n_layers_r: int = DEFAULT_LAYERS_R,
                          n_units_s_r: int = DEFAULT_DIM_S_R,
                          n_units_p_r: int = DEFAULT_DIM_P_R,
                          private_out: bool = False, binary_y: bool = False,
                          shared_repr: bool = False, same_init: bool = True):
    if n_layers_out < 1:
        raise ValueError("FlexTENet needs at least one hidden output layer (else there are no "
    ...
    if has_body:
        ...
        if shared_repr:  # fully shared representation as in TARNet
            ...
            layers = (Layer(n_units_s_r, n_units_p_r, first_layer=True, same_init=same_init), Nonlin_Elu)

            if n_layers_r > 1:
                # add required number of layers
                for i in range(n_layers_r - 1):
                    layers = (*layers, Layer(n_units_s_r, n_units_p_r, same_init=same_init), Nonlin_Elu)
    else:
        layers = ()

    # add output layers
    first_layer = (has_body == False) | (shared_repr == True)
    layers = (*layers, Layer(n_units_s_out, n_units_p_out, first_layer=first_layer, same_init=same_init),
              Nonlin_Elu)

    if n_layers_out > 1:
        # add required number of layers
        for i in range(n_layers_out - 1):
            layers = (*layers, Layer(n_units_s_out, n_units_p_out, same_init=same_init), Nonlin_Elu)
...
def elementwise_split(fun, **fun_kwargs):
...
def elementwise_parallel(fun, **fun_kwargs):
    """Layer that applies a scalar function elementwise on its inputs. Adapted from original
    ...
    """
    init_fun = lambda rng, input_shape: (input_shape, ())
    apply_fun = lambda params, inputs, **kwargs: (fun(inputs[0], **fun_kwargs),
...
def DenseW(out_dim, W_init=glorot_normal(), b_init=normal()):
