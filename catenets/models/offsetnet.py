...
from catenets.models.base import OutputHead, BaseCATENet
...
def predict_offsetnet(X, trained_params, predict_funs, return_po: bool = False,
...
def train_offsetnet(X, y, w, binary_y: bool = False, n_layers_r: int = DEFAULT_LAYERS_R,
                    n_units_r: int = DEFAULT_UNITS_R, n_layers_out: int = DEFAULT_LAYERS_OUT,
                    n_units_out: int = DEFAULT_UNITS_OUT, penalty_l2: float = DEFAULT_PENALTY_L2,
                    penalty_l2_p: float = DEFAULT_PENALTY_L2,
                    step_size: float = DEFAULT_STEP_SIZE,
                    ...
                    nonlin: str = DEFAULT_NONLIN, avg_objective: bool = True):
    # input check
    ...
    init_fun_head, predict_fun_head = OutputHead(n_layers_out=n_layers_out,
                                                 ...
                                                 binary_y=False, n_layers_r=n_layers_r,
                                                 n_units_r=n_units_r, nonlin=nonlin)

    def init_fun_offset(rng, input_shape):
    ...
    if not binary_y:
        @jit
        def loss_offsetnet(params, batch, penalty, penalty_l2_p):
            ...
            weightsq_head = heads_l2_penalty(params[0], params[1], n_layers_out + n_layers_r, False,
                                         penalty, penalty_l2_p)
        ...
        def loss_offsetnet(params, batch, penalty, penalty_l2_p):
            ...
            if not avg_objective:
                return - jnp.sum((targets * jnp.log(preds) +  (1 - targets) * jnp.log(
                        1 - preds))) + 0.5 * weightsq_head
            else:
                n_batch = y.shape[0]
                return - jnp.sum((targets * jnp.log(preds) +(1 - targets) * jnp.log(
                        1 - preds)))/n_batch + 0.5 * weightsq_head

    # Define optimisation routine
    opt_init, opt_update, get_params = optimizers.adam(step_size=step_size)

    @jit
    def update(i, state, batch, penalty_l2, penalty_l2_p):
        # updating function
