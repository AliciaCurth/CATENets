...
from catenets.models.snet import train_snet, predict_snet
from catenets.models.offsetnet import train_offsetnet, predict_offsetnet
from catenets.models.flextenet import train_flextenet, predict_flextenet
from catenets.models.tnet import train_tnet, predict_t_net
...
ALL_STRATEGIES = [T_STRATEGY, S1_STRATEGY, S2_STRATEGY, S3_STRATEGY, S_STRATEGY, FLEX_STRATEGY,
                  OFFSET_STRATEGY]
...
def _train_and_predict_first_stage(X, y, w, fit_mask, pred_mask, first_stage_strategy,
    ...
    if first_stage_strategy == T_STRATEGY:
        ...
        raise ValueError("{} is not a valid first stage strategy for a PseudoOutcomeNet".format(
            first_stage_strategy))

    if verbose > 0:
        print('Training PO estimators')
    trained_params, pred_fun = train_fun(X_fit, y_fit, w_fit, binary_y=binary_y,
    ...
    if first_stage_strategy in [S_STRATEGY, S2_STRATEGY, S3_STRATEGY]:
        _, mu_0, mu_1, pi_hat = predict_fun(X_pred, trained_params, pred_fun, return_po=True,
                                            return_prop=True)
    else:
