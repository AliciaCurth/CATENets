...
from catenets.models.base import train_output_net_only, BaseCATENet, OutputHead
...
class TNet(BaseCATENet):
    ...
    """
    def __init__(self, binary_y: bool = False, n_layers_out: int = DEFAULT_LAYERS_OUT,
                 n_units_out: int = DEFAULT_UNITS_OUT, n_layers_r: int = DEFAULT_LAYERS_R,
                 n_units_r: int = DEFAULT_UNITS_R,
                 penalty_l2: float = DEFAULT_PENALTY_L2, step_size: float = DEFAULT_STEP_SIZE,
                 ...
                 seed: int = DEFAULT_SEED, train_separate: bool = True,
...
def train_tnet(X, y, w, binary_y: bool = False, n_layers_out: int = DEFAULT_LAYERS_OUT,
               ...
               train_separate: bool = True, penalty_diff: float = DEFAULT_PENALTY_L2,
    ...
    if train_separate:
        ...
        out_0 = train_output_net_only(X[w == 0], y[w == 0],
                                      binary_y=binary_y,
        ...
        out_1 = train_output_net_only(X[w == 1], y[w == 1],
                                      binary_y=binary_y,
                                      n_layers_out=n_layers_out,
                                      n_units_out=n_units_out,
                                      n_layers_r=n_layers_r,
        ...
        params, predict_fun = _train_tnet_jointly(X, y, w, binary_y=binary_y,
                                                  n_layers_out=n_layers_out,
                                                  n_units_out=n_units_out,
                                                  n_layers_r=n_layers_r,
                                                  n_units_r=n_units_r,
                                                  ...
                                                  n_iter_print=n_iter_print,
                                                  verbose=verbose,
                                                  seed=seed, return_val_loss=return_val_loss,
                                                  penalty_diff=penalty_diff, nonlin=nonlin)
...
def predict_t_net(X, trained_params, predict_funs, return_po: bool = False,
...
def _train_tnet_jointly(X, y, w, binary_y: bool = False, n_layers_out: int = DEFAULT_LAYERS_OUT,
                        ...
                        verbose: int = 1, n_iter_print: int = DEFAULT_N_ITER_PRINT,
                        seed: int = DEFAULT_SEED, return_val_loss: bool = False,
                        same_init: bool = True, penalty_diff: float = DEFAULT_PENALTY_L2,
                        nonlin: str = DEFAULT_NONLIN, avg_objective: bool = DEFAULT_AVG_OBJECTIVE):
    # input check
    ...
    init_fun_head, predict_fun_head = OutputHead(n_layers_out=n_layers_out, n_units_out=n_units_out,
    ...
    def loss_tnet(params, batch, penalty_l2, penalty_diff):
        ...
        # regularization
        weightsq_head = heads_l2_penalty(params[0], params[1], n_layers_r + n_layers_out, True,
